...
import s from '../../components/RatingItem/RatingItem.module.scss'
import {useFetchUserQuery} from "../../redux/auth/authApiSlice.js";
import deleteIcon from './../../assets/trash.svg'
import {useDeleteEventRatingMutation, useCreateEventRatingAnswerMutation,
    useUpdateEventRatingAnswerMutation,
    useDeleteEventRatingAnswerMutation,} from "../../redux/event/event.api.js";
import {useInput} from "../../utils.js";
const VITE__API = import.meta.env.VITE__API
const AnswerCreate = ({id, onClose})=>{
    const [text, setText] = useInput('')
    const [create] = useCreateEventRatingAnswerMutation()
...
const AnswerItem = ({data,ratingId})=>{
    ...
    const [update] = useUpdateEventRatingAnswerMutation()
    const [remove] = useDeleteEventRatingAnswerMutation()
...
const RatingItem = ({data}) => {
    ...
    const [deleteRating] = useDeleteEventRatingMutation()
    const handleDelete = async () =>{
        try{
            console.log(data._id)
           const res = await deleteRating({id:data._id})
            console.log({res})
        }catch (e) {
            console.log(e)
        }
